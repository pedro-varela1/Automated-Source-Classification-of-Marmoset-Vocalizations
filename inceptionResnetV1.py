...
class Block8(nn.Module):
    ...
    def __init__(self, scale=1.0, noReLU=False, cam=False):
        ...
        if cam:
            # If cam, conv2d have more kernel size
            self.conv2d
        else:
            self.conv2d = nn.Conv2d(384, 1792, kernel_size=20, stride=1)
        if not self.noReLU:
            self.relu = nn.ReLU(inplace=False)
...
class InceptionResnetV1(nn.Module):
    ...
    Keyword Arguments:
        ...
        rgb {bool} -- Whether the model expects RGB input images. If False, model expects
            grayscale images. (default: {False})
        num_classes {int} -- Number of output classes. (default: {None})
    ...
    def __init__(self, classify=False, rgb=False, num_classes=None, cam=False, dropout_prob=0.7, embeddings_size=512, device=None):
        ...
        if self.rgb:
            ...
            self.conv2d_1a = BasicConv2d(1, 32, kernel_size=3, stride=2)    # Greyscale
        ...
        if self.classify and self.num_classes is not None:
            self.logits = nn.Linear(self.embeddings_size, self.num_classes)
